#include "livox_undistortion/gyr_int.hpp"
...
void GyrInt::Integrate(const sensor_msgs::msg::Imu::ConstPtr &imu)
{
  ...
  if (v_rot_.empty()) {
    if (start_timestamp_ <= 0) {
      throw std::runtime_error("ERROR: start_timestamp_ must be greater than 0");
    }
    if (last_imu_ == nullptr) {
      throw std::runtime_error("ERROR: last_imu_ cannot be nullptr");
    }
    ...
    double dt1 = start_timestamp_ - get_time_sec(last_imu_->header.stamp);
    double dt2 = get_time_sec(imu->header.stamp) - start_timestamp_;
    if (dt1 < 0 || dt2 < 0) {
      char buffer[100];
      snprintf(buffer, sizeof(buffer), "%f - %f - %f",
               get_time_sec(last_imu_->header.stamp), start_timestamp_,
